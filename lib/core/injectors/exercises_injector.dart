...
AutoInjector exercisesInjectorModule() => AutoInjector(
  tag: 'exercisesInjectorModule',
  on: (i) {
    // 1. Camada de Network (External)
    i.addSingleton<IApiManager>(
      () => NetworkConfig.create(baseUrl: 'https://api.api-ninjas.com'),
    );

    i.addSingleton<ExercisesDatasource>(ExercisesDatasource.new);

    i.addSingleton<IExercisesRepository>(ExercisesRepositoryImpl.new);
